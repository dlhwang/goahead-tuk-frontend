...
flowchart LR
    ...
    Frontend --> DeviceStore["브라우저 localStorage 식별 및 반응 기록"]
    API --> Frontend
```
...
2. 프론트엔드는 생성한 device id와 성공한 반응 선택 기록을 브라우저
   `localStorage`에 저장한다.
3. 프론트엔드는 외부 REST API를 통해 고해 데이터를 조회, 생성하고 반응을
   선택하거나 해제한다.
...
- **비즈니스 트랜잭션**:
  ...
  - 고해 반응 남기기: 목록 또는 상세 화면에서 기도, 토닥임, 함께함 반응을
    선택하거나 해제한다.
- **비즈니스 용어**:
  - **Confession**: mood, 생성 시각, 타입별 반응 집계를 가진 사용자 작성
    텍스트 항목.
  - **Mood**: 고해에 부여되는 지원 감정 범주 중 하나.
  - **Comfort message**: 고해 상세 응답에 포함될 수 있는 선택적 위로 문구.
  - **Device id**: `X-Device-Id` 헤더로 전송되는 브라우저 범위 식별자.
  - **Reaction**: `PRAY`, `COMFORT`, `TOGETHER` 중 하나인 공감 표현과
    현재 집계 수.
...
- **책임**: 로딩 상태, 오류 복구, 빈 상태, 카드 렌더링, 새로고침,
  타입별 반응 표시와 반응 조작 진입점.
...
- **목적**: 전체 고해, API가 반환한 위로 응답, 타입별 반응을 보여준다.
