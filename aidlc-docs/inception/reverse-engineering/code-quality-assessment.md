...
- **Linting**: 설정되어 있고 2026-05-27에 `npm.cmd run lint`가 성공했다.
- **Build 검증**: 2026-05-27에 `npm.cmd run build`가 성공했다.
...
- backend 응답은 반응의 현재 device 선택 여부를 제공하지 않는다.
  `reactionSelections.ts`는 성공한 로컬 동작을 기억하지만 서버 또는 다른
  브라우저에서 변경된 선택 여부를 복원하는 근거가 되지 않는다.
- 사용자 조작 버튼에 안정적인 `data-testid` 속성이 없어 향후 자동화
  검증의 selector 안정성을 별도로 보강할 필요가 있다.
...
- **좋은 패턴**:
  ...
  - 반응 type과 label이 단일 model 상수에서 관리되며 누락된 count를
    방어적으로 `0` 처리한다.
...
- **안티패턴**:
  ...
  - 반응 선택 표시가 backend가 확인한 선택 상태가 아니라 로컬 기록에
    의존한다.
